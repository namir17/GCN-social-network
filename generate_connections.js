...
/**
 * Function to create realistic social connections between users.
 */
async function createRealisticConnections() {
  const session = driver.session();

  try {
    // Fetch users and their hobbies from the database
    ...
    for (const userA of users) {
      ...
      // Filter potential connection candidates based on common interests and connection limits
    ...
    for (let i = 0; i < users.length * maxConnectionsPerUser * 0.1; i++) { // Add 10% random connections
      ...
      if (userA.id === userB.id ||
          userCounters[userA.id].total >= maxConnectionsPerUser || userCounters[userB.id].total >= maxConnectionsPerUser) {
            continue;
          }
